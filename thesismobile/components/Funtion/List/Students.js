...
const Students = () => {
    ...
    const fetchThesisData = async (page, query) => {
        if (page > 0) {
            let url = `${endpoints['students']}?q=${query}&page=${page}`;
            try {
                setLoading(true);
    ...
    const RenderItem = memo(({ item }) => (
        <View style={ManagerStudyStyle.item} key={item.id}>
            <Text style={StyleStudy.itemText}>Mã Sinh Viên: {item.code}</Text>
            ...
            <Text style={StyleStudy.itemText}>GPA: {item.gpa}</Text>
            <Text style={StyleStudy.itemText}>
                Khóa luận: {item.thesis ? item.thesis : 'chưa làm khóa luận'}
            </Text>
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
            <View style={StyleStudy.container}>
                ...
                <View style={StyleStudy.TopBackGround}>
                    ...
                    <Text style={StyleStudy.greeting}>DANH SÁCH SINH VIÊN</Text>
                </View>
                <Searchbar placeholder="Tìm kiếm theo tên sinh viên..." value={searchQuery} onChangeText={search} style={ManagerStudyStyle.Search} />
                <ScrollView
                    onScroll={loadMore}
